...
const IllustratedMessage = ({ setShowCreateForm, errorMessage }) => {
  ...
  const textArray = Object.keys(errorMessage)?.filter(key => key?.startsWith('text'));
  ...
  return (
    ...
    >
      ...
      {textArray.length ?
        ...
        >
          {textArray.map((key, index) => (
            errorMessage[key] && <p key={index} className='spectrum-Body spectrum-Body--sizeM'>{errorMessage[key]}</p>
          ))}
        </div> : null
      }
      <button className="spectrum-Button spectrum-Button--outline spectrum-Button--primary spectrum-Button--sizeM" onClick={() => setShowCreateForm(true)}>
        <span className="spectrum-Button-label">{errorMessage?.buttonLabel}</span>
      </button>
